...
export class DualListBoxComponent implements OnInit, ControlValueAccessor {
  ...
  // Pagination cho data trong box
  @Input() pagerTreeView: any;
  ...
  // Check when tab is opened
  _openTab: string;
  @Input()
  set openTab(openTab: string) {
  ...
  @Output() currentPageTreeView: EventEmitter<number> = new EventEmitter<number>();
  ...
  setPage(number) {
    this.currentPageTreeView.emit(number);
  }
  ...
  moveMarkedAvailableItemsToSelected(): void {
    ...
    const values: any[] = this.availableListBoxControl.value || [];
    this.selectedItems = [...this.selectedItems,
    ..._.intersectionWith(this.availableItems, values, (item: IListBoxItem, value: string) => item.value === value)];
    // now filter available items to not include marked values
    this.availableItems = [..._.differenceWith(this.availableItems, values, (item: IListBoxItem, value: string) => item.value === value)];
  ...
  moveMarkedSelectedItemsToAvailable(): void {
    ...
    const values: any[] = this.selectedListBoxControl.value || [];
    // first move items to available
    this.availableItems = [...this.availableItems,
    ..._.intersectionWith(this.selectedItems, values, (item: IListBoxItem, value: string) => item.value === value)];
    // now filter available items to not include marked values
    this.selectedItems = [..._.differenceWith(this.selectedItems, values, (item: IListBoxItem, value: string) => item.value === value)];
    // clear marked available items and emit event
    this.onItemsMoved.emit({
      available: this.availableItems,
      selected: this.selectedItems,
  ...
  writeValue(value: any[]): void {
    if (this.selectedItems && value && value.length > 0) {
      this.selectedItems = [...this.selectedItems,
      ..._.intersectionWith(this.availableItems, value, (item: IListBoxItem, value2: string) => item.value === value2)];
      this.availableItems = [..._.differenceWith(this.availableItems, value, (item: IListBoxItem, value2: string) => item.value === value2)];
    }
