...
} from './input-type-text/input-type-text.component';
import { TtktInputService } from './ttkt.input.service';
import { MESSAGE_VALIDATION, MESSAGE_VALIDATION_RULE } from './../message.validation';
import { InputTypeBase } from './input-type.interface';

@Component({
  selector: 'ttkt-input',
  templateUrl: './ttkt-input.component.html',
  styleUrls: ['./ttkt-input.component.scss'],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => TtktInputComponent),
      multi: true
    }
  ]
})
export class TtktInputComponent implements OnInit, ControlValueAccessor, AfterViewInit, OnDestroy {
  // only have one type
  ...
  constructor(
    ...
    private helper: TtktInputService,
  ...
  initDataForChild(data: any) {
    if (this.componentRef && data) {
      ...
      if (this.componentRef.instance instanceof TtktInputTypeTextComponent) {
